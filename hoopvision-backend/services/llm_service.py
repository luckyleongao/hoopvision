...
def generate_coach_feedback(
    ...
    if not api_key:
        return f"【本地规则反馈】手肘{int(min_elbow_angle)}°，膝盖{int(min_knee_angle)}°，出手弧度{int(release_angle)}°。(提示: 请在环境变量配置 GEMINI_API_KEY)"
    ...
    star_profiles = {
        "curry": {
            ...
            "focus": "重点检查【下蹲蓄力与向上举球是否同步进行】、【起跳未至最高点即借力极速推射出手】、【出手抛物线高弧度(50°~55°)，柔和顺畅】。无需深蹲，重点是动量由下至上零停顿传递！"
        },
        "kobe": {
            "name": "科比·布莱恩特 (Kobe Bryant)",
            "style": "Two-Motion 滞空干拔跳投",
            "focus": "重点检查【深蹲起跳至最高点滞空瞬间才伸臂出手】、【手肘高位架起形成高出手点】、【出手仰角约46°~50°直插网窝】、【核心收紧与滞空稳定性】。"
        },
        "klay": {
            "name": "克莱·汤普森 (Klay Thompson)",
            "style": "教科书式标准定点投篮",
            "focus": "重点检查【托球手肘严格呈现 90 度 L 型标准直角】、【起跳垂直起落中轴线不歪斜】、【标准 48° 黄金出手弧度】、【辅助手干净不发力】。"
        }
    }
...
4. 实测篮球出手飞行仰角 (Release Angle): {int(release_angle)} 度 (黄金弧度区间为 45°~52°)。

【针对性教学重点】：
{star_info['focus']}

【任务指令】：
请仔细观察 3 张实拍照片中的身体姿态与手型，结合骨骼与出手弧度数据：
